...
class Version:
    ...
    def parse(version_str: str):
        version_pattern = (r"^"                   # Match from the start of the whole string
                           # Optional prefix (for debug reasons only)
                           r"(?P<prefix>.+)?"
                           r"(?P<major>\d+)\."    # Major
                           r"(?P<minor>\d+)\."    # Minor
                           r"(?P<patch>\d+)"      # Patch
                           # Postfix follows all candidates. Can include ASCII and underscore.
                           r"(-(?P<candidate>[a-zA-Z_]+)"
        ...
        assert type(version_str) == str, type(version_str)
    ...
    def __repr__(self):
        ret = "{}{}.{}.{}".format(
            self.prefix or "", self.major, self.minor, self.patch)
...
class VersionService:
    def __init__(self,
                 candidate_provider,
                 base_version_provider,
                 previous_versions_provider):
        """
    ...
    def tag(self, version):
        subprocess.check_call(
            ["git",
             "tag",
             "-a", str(version), "-m", "Auto tagged by release-tools"])
    ...
    def get_next_version(self):
        ...
        if not base_version:
            raise AssertionError(
                "Version provider is not returning a suggestion")
        if base_version.candidate:
            raise AssertionError(
                "The base version should not have a candidate marker")
        if base_version.candidate_seq:
            raise AssertionError(
                "The base version should not have a candidate sequence")
...
class ConstantCandidateProvider:
...
class VersionFileInPythonPackageBaseVersionProvider:
    def __init__(self):
        pass
    ...
    def get_version(self):
        ...
        Returns the target version (the one we're aiming for after release candidates) as a string
        """
        package = find_single_package()

        with open(os.path.join(package, "VERSION")) as fs:
            return fs.read()
